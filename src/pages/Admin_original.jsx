  ...
  const createProject = async (e) => {
    ...
    try {
      ...
      const projectData = {
        ...
        expectedReturnRate: parseFloat(projectForm.expectedReturnRate), // Mantener como porcentaje
        fixedAmount: fixedAmount,
        endDate: projectForm.endDate
      };
      
      // Solo incluir withdrawalRestrictionPercentage si tiene un valor válido
      // Esto permite compatibilidad con backends que aún no soportan este campo
      if (withdrawalRestriction >= 0 && withdrawalRestriction <= 100) {
        // Enviar como porcentaje directo
                ...
                <div className="flex flex-col items-center space-y-4">
                  <div className="w-16 h-16 bg-gradient-to-r from-purple-600/30 to-blue-600/30 rounded-full flex items-center justify-center">
                    <svg className="w-8 h-8 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h4a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                  </div>
      ...
      >
        <form onSubmit={createProject} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="modal-withdrawalRestrictionPercentage" className="block text-sm font-medium text-gray-700 mb-2">
                Restricción de Retiro (%) <span className="text-gray-400">(Opcional)</span>
              </label>
              <input
                type="number"
                id="modal-withdrawalRestrictionPercentage"
                ...
                max="100"
                className="w-full px-4 py-3 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200"
                placeholder="Ej: 70.0 (dejar vacío para usar valor por defecto)"
              />
              <p className="text-xs text-gray-500 mt-1">
                Porcentaje del monto invertido que no se puede retirar (0-100%). Si se deja vacío, se usará el valor por defecto del backend.
              </p>
            </div>

            {/* Monto Fijo del Proyecto */}
            <div>
              <label htmlFor="modal-fixedAmount" className="block text-sm font-medium text-gray-700 mb-2">
                Monto Fijo del Proyecto (COP) *
              </label>
              <input
                type="number"
                id="modal-fixedAmount"
                name="fixedAmount"
